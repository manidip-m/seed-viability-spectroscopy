...
- `NIR_new.m` — main analysis pipeline for the NIR dataset
- `Raman_new.m` — main analysis pipeline for the Raman dataset
...
- `Raman_wdf_conv_new.m` — converts raw Renishaw `.wdf` Raman scans into the single `RamanDataset_Processed.mat` file used by `Raman_new.m`, applying basic quality control (drops spectra with weak signal or excessive saturation) along the way 
...
- A `WdfReader`-capable MATLAB setup (typically via Renishaw's WiRE software or their published file-format toolkit) — only needed to run `Raman_wdf_conv_new.m`; not required for `Raman_new.m` itself
...
2. **(Raman only, from raw data)** If starting from raw `.wdf` scans rather than an existing `.mat` file, run `Raman_wdf_conv_new.m` first to produce `RamanDataset_Processed.mat`. Update the folder paths near the top of the script to point to your own raw scan folders first.
3. Open `NIR_new.m` or `Raman_new.m` in MATLAB.
